<script lang="ts">
	import { boardFromInput, posToAlgebraic, type Square } from '$lib/chess/board';

	let { board, fen, onMove, flipped = false, lastMove, disabled = false }: {
		board?: string[];
		fen?: string;
	...
	const PIECE_IMG: Record<string, string> = {
		K: '/pieces/wK.webp', Q: '/pieces/wQ.webp', R: '/pieces/wR.webp',
		B: '/pieces/wB.webp', N: '/pieces/wN.webp', P: '/pieces/wP.webp',
		k: '/pieces/bK.webp', q: '/pieces/bQ.webp', r: '/pieces/bR.webp',
		b: '/pieces/bB.webp', n: '/pieces/bN.webp', p: '/pieces/bP.webp',
	};
...
<div class="inline-grid grid-cols-8 select-none" style="width: min(100%, 30rem); aspect-ratio: 1;">
	{#each getRows() as row, r}
		{#each row as sq, c}
			<button
				class="flex items-center justify-center relative aspect-square transition-colors {cellClass(r, c)} {sq.piece && !disabled ? 'cursor-pointer' : ''}"
			...
			>
				...
				{/if}
				{#if c === 0}
					<span class="absolute top-0.5 left-1 text-xs font-semibold {sq.isLight ? 'text-board-dark' : 'text-board-light'}">
						{flipped ? ranks[7 - r] : ranks[r]}
					</span>
				{/if}
				{#if r === 7}
					<span class="absolute bottom-0.5 right-1 text-xs font-semibold {sq.isLight ? 'text-board-dark' : 'text-board-light'}">
