	...
	<div class="space-y-6">
		<section class="card">
			...
			{#if elo !== null || points !== null}
				<div class="grid {elo !== null && points !== null ? 'grid-cols-2' : 'grid-cols-1'} gap-3 mb-3">
					...
					{#if points !== null}
						<div class="text-center bg-primary-transparent2 rounded p-2">
							<div class="text-xl font-bold text-primary">{formatPoints(points)} PPP</div>
		...
		{#if activeGame}
			<section>
				...
				<a href="/game/{encodeURIComponent(JSON.stringify(activeGame.game_id))}">
					<div class="card flex items-center justify-between">
						<div class="text-sm">
							<span class="inline-block w-3 h-3 rounded-full bg-white mr-1 align-middle"></span>
							{activeGame.white.type === 'Human' ? activeGame.white.value : 'AI'}
							<span class="text-white/40 mx-1">vs</span>
							{activeGame.black?.type === 'Human' ? activeGame.black.value : 'AI'}
						</div>
				...
				<div class="space-y-1.5">
					{#each questCooldowns as [ts, quest]}
						<div class="flex justify-between items-center text-sm">
							...
							<span class="text-xs {formatCooldown(ts) === 'Ready' ? 'text-primary-green' : 'text-white/50'}">
								{formatCooldown(ts)}
							</span>
						</div>
					{/each}
			...
			<section class="card">
				...
				<div class="flex flex-wrap gap-2">
					{#each achievements as [ts, ach]}
						<span class="text-xs px-2 py-1 rounded bg-primary-transparent2 text-primary border border-primary/30">
